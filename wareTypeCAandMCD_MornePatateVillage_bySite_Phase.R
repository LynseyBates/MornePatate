...
# master context number and unit
#Create DF of only unit and ProjectID
Block <- unique(wareTypeData_Unit %>% select(MasterContextNumber, unit))

rowScoresX <- merge(rowScores, Block, by="unit")
...
# Rerun CA
matX2 <- as.matrix(WareByUnitTY[,-1])
rownames(matX2) <- WareByUnitTY$unit
...
rowScores2 <- data.frame(ca2$rowcoord[,1:5], unit =ca2$rownames)
colScores2 <- data.frame(ca2$colcoord[,1:5], type =ca2$colnames)
...
p <- ggplot(data=inertia , aes(x= 1:length(Inertia), y=Inertia)) +
  ...
  geom_line(col= "cornflower blue", size=1) +
  geom_point(shape=21, size=5, colour="black", fill="cornflower blue") +
  labs( title="Morne Patate Village No P01", x="Dimension", y='Proportion of Inertia' ) +
  geom_line(aes(y = bs[,2], x= bs[,1]), color = "black", linetype = "dashed", 
            size=1)
p
...
rowScoresX <- merge(rowScores2, Block, by="unit")

# ggplot version of row scores dim 1 and dim 2
library(ggrepel)
set.seed(42)
p1a <- ggplot(rowScoresX, aes(x=Dim1,y=Dim2, fill=MasterContextNumber))+
...
#ggplot version of col scores dim 1 and dim 2
p2a <- ggplot(colScores2, aes(x = Dim1,y = Dim2))+
  geom_point(shape=21, size=5, colour="black", fill="cornflower blue")+
  #geom_text(aes(label= type),vjust=-.6, cex=5)+
  geom_text_repel(aes(label=type), cex= 3) +
  labs(title="Morne Patate Village", 
...
#### 14. Compare MCD and CA dim scores
# create a data frame of units, counts, and mcds
CA_MCD2 <- inner_join(MCDByUnit$MCDs, rowScores2, by='unit' )

# Plot CA Dim 1 vs. MCDs
#ggplot version of CA Dim 1 vs. MCDs
p3a <- ggplot(CA_MCD2, aes(x=Dim1,y=blueMCD))+
  geom_point(shape=21, size=5, colour="black", fill="cornflower blue")+
  #geom_text(aes(label=unit),vjust=-.6, cex=5)+
  #geom_text_repel(aes(label=unit), cex=6) +
  labs(title="Morne Patate Village No P01", 
       x="Dimension 1", 
       y="BLUE MCD") 
p3a 
# save the plot for website chronology page/presentations
...
#### 15. Histogram of Dim 1 scores for Phasing
# Dim 1 Scores Weighted Histogram, you may need to change scale
dim1ForHist<- data.frame(dim1 = rep(CA_MCD2$Dim1, CA_MCD2$Count))
p5 <- ggplot(dim1ForHist, aes(x = dim1)) +
  geom_histogram(aes(y=..density..), colour="black", fill="tan", binwidth=0.1, 
                 boundary= .1) +
  scale_x_continuous(breaks=seq(- 3 , 3, 0.5))+
  labs(title="Morne Patate Village No P01", x="Dimension 1", y="Density") +
  geom_density(fill=NA)
p5


# Add lines for phase breaks
p5a <- p5 + geom_vline(xintercept=c(- 1, 1 ), colour = "gray", linetype = "dashed",
                       size=1)
p5a
...
#### 16.  Do the Dim 1 -  MCD scatterplot with Phase assignments  
# Do the Phase assigments, based on the Dim1 scores
CA_MCD_Phase3 <- CA_MCD2 %>% mutate( Phase = case_when (Dim1 <= -1 ~ 'P02',
   (Dim1 > -1) & 
  (Dim1 <= 1) ~ 'P03',
  Dim1 > 1 ~ 'P04'
))
